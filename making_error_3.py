	...
	while Fraq < 0.05 :
		...
		for k in range(3):
			for i in range(32):
				for j in range(32):
					if i==0 and j==0:
						if abs(tmp[i][j][k]-tmp[i+1][j][k]) > threshold or abs(tmp[i][j][k]-tmp[i][j+1][k]) > threshold:
							Y[i][j][k] = 255
							Gmap[i][j] = 255
							count += 1
						...
						if abs(tmp[i][j][k]-tmp[i+1][j][k]) > threshold or abs(tmp[i][j][k]-tmp[i][j+1][k]) > threshold or abs(tmp[i][j][k]-tmp[i][j-1][k]) > threshold:
							Y[i][j][k] = 255
							Gmap[i][j] = 255
							count += 1
					elif i==31:
						...
						if abs(tmp[i][j][k]-tmp[i+1][j][k]) > threshold or abs(tmp[i][j][k]-tmp[i+1][j][k]) > threshold or abs(tmp[i][j][k]-tmp[i][j+1][k]) > threshold:
							Y[i][j][k] = 255
							Gmap[i][j] = 255
							count += 1
					elif j==31:
						...
						if abs(tmp[i][j][k]-tmp[i-1][j][k]) > threshold or abs(tmp[i][j][k]-tmp[i+1][j][k]) > threshold or abs(tmp[i][j][k]-tmp[i][j-1][k]) > threshold or abs(tmp[i][j][k]-tmp[i][j+1][k]) > threshold:
							Y[i][j][k] = 255
							Gmap[i][j] = 255
							count += 1
						
		Fraq=count/32.0/32.0/3.0
...
#	store them in dataset[0~9999]
	...
	for k in range(len(dataset)):
		...
		#show(dataset[k])
		#im = plt.imshow(Gradient(dataset[k], k) )
		#plt.show()
